...
uint8_t kbd_code = 0x00;


int (kbd_subscribe_int)(uint8_t *bit_no) {
  *bit_no = BIT(kbd_id); //bits_no = 1

  if(sys_irqsetpolicy(KBD_IRQ, IRQ_REENABLE | IRQ_EXCLUSIVE, &kbd_id)!=0){
...
void (kbc_ih)(void) {
  ...
  if((ctrl & STAT_REG_OBF) != 0 || (ctrl & (STAT_REG_PARITY | STAT_REG_TIMEOUT)) != 0){ //Check if Output Buffer is full or if Parity of Timeout Error
    if(util_sys_inb(OUT_BUF, &kbd_code) != 0){ //Check if Error getting ScanCode from Output Buffer
      kbd_code = 0x00;
      return;
    }
  }
